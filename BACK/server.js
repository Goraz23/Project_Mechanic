...
// ! Configuracion de la variable global
let codigo;
...
// ! ENVIAR CORREO Y CODIGO DE VERIFICACIÓN USANDO  NODEMAILER
...
const enviarMail = async(destinatario, coding) => {
  ...
  const mensaje = {
      ...
      text: `!Hola! Detectamos que estas intentando iniciar sesion en Mechanic, para confirmar que eres tu, ingresa el siguiente codigo: 
      | "${coding}" |`
...
connection.query(
  `SELECT mechanic.id_mechanic, mechanic.rol_id, mechanic.pass FROM mechanic WHERE email =? AND pass=?`,
  [email,password],
  async (error, result) => {
    if (result.length === 0) {
      ...
      const userId = result[0].id_mechanic;
      response.status(200).json({
        respuesta: result[0],
        status: true,
      });
      ...
      connection.query(
        "UPDATE mechanic SET codigo = ? WHERE id_mechanic = ?;",
        [codigo, userId],
        (errors, results) => {
          if (errors) {
            console.error(
  ...
  (error, results) => {
    if (error) {
      ...
      if (results.length > 0) {
        ...
        response.status(200).json({ status: true, acceso: true, success: true, alias:results[0].alias, surname: results[0].surname , role: results[0].rol_id, token:'isAuth' });
      } else {
